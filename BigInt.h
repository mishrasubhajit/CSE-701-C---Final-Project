 ...
 * unsigned long long int type used for indexing (the "length" property). 
 ...
 * When creating a BigInt from const char* or unsigned long long int, 
 ...
 * 
 * 	- automatic conversion from const char *, std::string and unsigned long long int
...
private:
	...
	// Number of digits
	unsigned long long int digitCount;
	// Sign
	bool positive;
	...
	static unsigned long long int int2uchar(unsigned long long int number,
											unsigned char *digits,
	...
	static int compareNumbers(unsigned char *a, unsigned long long int na,
	...
	static void longMultiply(unsigned char *a, unsigned long long int na,
							 unsigned char *b, unsigned long long int nb,
	...
	static unsigned long long int toInt(unsigned char *digits, int n);
	...
	static bool add(unsigned char *shorter, unsigned long long int nShorter,
					unsigned char *longer, unsigned long long int nLonger,
					unsigned char *result, int nResult,
					bool doFill = true);
	/* Shifts the digits n places left. */
	BigInt &shiftLeft(unsigned long long int n);
	/* Shifts the digits n places right. */
	BigInt &shiftRight(unsigned long long int n);
	/* Expands the digits* to n. */
	void expandTo(unsigned long long int n);

public:
	BigInt();
	BigInt(const char *charNum);
	BigInt(unsigned long long int intNum);
	...
	BigInt GetPower(unsigned long long int n) const;
	/* *this = *this to the power of n. */
	void SetPower(unsigned long long int n);
	/* Returns *this to the power of n 
		 * using the fast Square and Multiply algorithm. */
	...
	unsigned char GetDigit(unsigned long long int index) const;
	/* Sets the value of 'index'th digit 
		 * (zero-based, right-to-left) to 'value'. */
	void SetDigit(unsigned long long int index, unsigned char value);
...
/* Returns the number of digits. */
inline unsigned long long int BigInt::Length() const
{
	return digitCount;
}
